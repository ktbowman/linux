...
__unwind_incomplete_requests(struct intel_engine_cs *engine)
{
	...
	list_for_each_entry_safe_reverse(rq, rn,
		...
		/* LYUDE FIXME: This code is very clearly broken - upstream pl is uninitialized
		 * when declared, and pl only gets initialized if rq_prio(rq) != prio is ever hit.
		 * It's very unclear what the behavior on no pl should even be here, so punt this
		 * bug off to ickle for the time being and just scream if we ever hit this code in
		 * the real world
		 */
		BUG_ON(!pl);
...
int intel_execlists_submission_setup(struct intel_engine_cs *engine)
{
	...
	tasklet_init(&engine->execlists.tasklet, execlists_submission_tasklet,
...
{
	struct virtual_engine * const ve = (struct virtual_engine * const)data;
			       ...
			       unsigned int count)
{
	...
	ve->base.execlists.queue_priority_hint = INT_MIN;
	tasklet_init(&ve->base.execlists.tasklet, virtual_submission_tasklet,
		     (unsigned long)ve);
