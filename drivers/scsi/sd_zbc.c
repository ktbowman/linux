			       ...
			       unsigned int idx, report_zones_cb cb, void *data)
{
	...
	if (sdkp->aux->rev_wp_offset)
		sdkp->aux->rev_wp_offset[idx] = sd_zbc_get_zone_wp_offset(&zone);
				    ...
				    void *data)
{
	...
	lockdep_assert_held(&sdkp->aux->zones_wp_offset_lock);

	sdkp->aux->zones_wp_offset[idx] = sd_zbc_get_zone_wp_offset(zone);
					...
					unsigned int nr_blocks)
{
	...
	case SD_ZBC_INVALID_WP_OFST:
		...
		sdkp->aux->zones_wp_offset[zno] = SD_ZBC_UPDATING_WP_OFST;
		schedule_work(&sdkp->aux->zone_wp_offset_work);
		//fallthrough;
					  ...
					  unsigned int good_bytes)
{
	...
	if (result && op != REQ_OP_ZONE_RESET_ALL) {
		 ...
		 */
		if (sdkp->aux->zones_wp_offset[zno] != SD_ZBC_UPDATING_WP_OFST)
			sdkp->aux->zones_wp_offset[zno] = SD_ZBC_INVALID_WP_OFST;
		goto unlock_wp_offset;
	}

	switch (op) {
	case REQ_OP_ZONE_APPEND:
		rq->__sector += sdkp->aux->zones_wp_offset[zno];
		//fallthrough;
	case REQ_OP_WRITE_ZEROES:
	case REQ_OP_WRITE_SAME:
	case REQ_OP_WRITE:
		if (sdkp->aux->zones_wp_offset[zno] < sd_zbc_zone_sectors(sdkp))
			sdkp->aux->zones_wp_offset[zno] +=
