...
static int add_host_bridge_dport(struct device *match, void *arg)
{
	...
	/*
	 * In RCH mode, bind the component regs to the dport. In VH
	 * mode it is bound to the CXL host bridge's port object later
...
static int add_host_bridge_uport(struct device *match, void *arg)
{
	...
	if (ctx.base && ctx.cxl_version != ACPI_CEDT_CHBS_VERSION_CXL11 )
