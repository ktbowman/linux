...
mlxsw_sp_nexthop6_group_hash(struct mlxsw_sp_fib6_entry *fib6_entry, u32 seed)
{
	...
	list_for_each_entry(mlxsw_sp_rt6, &fib6_entry->rt6_list, list) {
		...
		struct in6_addr *gw = &fib6_nh->nh_gw;
					  ...
					  struct mlxsw_sp_fib6_entry *fib6_entry)
{
	...
	list_for_each_entry(mlxsw_sp_rt6, &fib6_entry->rt6_list, list) {
		...
		if (nh && nh->offloaded)
			fib6_nh->nh_flags |= RTNH_F_OFFLOAD;
		else
			fib6_nh->nh_flags &= ~RTNH_F_OFFLOAD;
	}
}
				    ...
				    unsigned long event, struct fib_nh *fib_nh)
{
	...
	case FIB_EVENT_NH_ADD:
		mlxsw_sp_nexthop4_type_init(mlxsw_sp, nh, fib_nh->nh_dev);
		break;
...
mlxsw_sp_nexthop4_group_info_init(struct mlxsw_sp *mlxsw_sp,
				  struct mlxsw_sp_nexthop_group *nh_grp)
{
	unsigned int nhs = nh_grp->ipv4.fi->fib_nhs;
	struct mlxsw_sp_nexthop_group_info *nhgi;
	struct mlxsw_sp_nexthop *nh;
	int err, i;
	...
	for (i = 0; i < nhgi->count; i++) {
		...
		nh = &nhgi->nexthops[i];
		fib_nh = &nh_grp->ipv4.fi->fib_nh[i];
...
{
	struct net *net = mlxsw_sp_net(mlxsw_sp);
	bool usp = net->ipv4_sysctl_ip_fwd_update_priority;
