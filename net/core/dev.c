...
{
	if (!skip_schedule) {
		// gro_normal_list(napi); -- not in RHEL 8
		__napi_schedule(napi);
		return;
	}

	if (!list_empty(&napi->gro_list)) {
...
static int dev_xdp_alloc_state(struct net_device *dev)
{
	if (!dev->xdp_state) {
		dev->xdp_state = kzalloc(sizeof(struct bpf_xdp_entity) * __MAX_XDP_MODE,
					 GFP_USER);
		if (!dev->xdp_state)
			return -ENOMEM;
	}
...
/* RHEL: any call to dev_xdp_set_link with non-NULL link must be preceded by
 * a call to dev_xdp_alloc_state.
 */
static void dev_xdp_set_link(struct net_device *dev, enum bpf_xdp_mode mode,
			     struct bpf_xdp_link *link)
{
	if (!dev->xdp_state && !link)
		return;
	dev->xdp_state[mode].link = link;
	dev->xdp_state[mode].prog = NULL;
}

/* RHEL: any call to dev_xdp_set_prog with non-NULL prog must be preceded by
 * a call to dev_xdp_alloc_state.
 */
static void dev_xdp_set_prog(struct net_device *dev, enum bpf_xdp_mode mode,
			     struct bpf_prog *prog)
{
	if (!dev->xdp_state && !prog)
		return;
